...
describe('pluck-or', () => {
	...
	it(
        'should pluck the backup item from a source if the initial item does not exist',
        async () => {
            const observable = of({
                message: 'Like and subscribe!',
            } as any);
	...
	it('should return undefined if the source does not exist', async () => {
		const observable = of(undefined as any);
	...
	it(
        'should return undefined if the item and backup item was not found',
        async () => {
            const observable = of({ hello: 'world' } as any);
